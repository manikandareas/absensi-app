...
export const columns: ColumnDef<Students>[] = [
  {
    ...
    header: "No",
    cell: ({ row }) => (
      <div className="text-left capitalize">{row.index + 1}</div>
    ),
  },
  {
    accessorKey: "name",
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      //   const time = intlFormatDistance(new Date(createdAt), new Date());
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            <DropdownMenuItem asChild>
              <Link href={`/users/${row.getValue("userId")}`}>
                View profile
              </Link>
            </DropdownMenuItem>
          </DropdownMenuContent>
...
export function ListPresencesTable(props: { presences: Students[] }) {
  // const data = React.useCallback(() => {
  //   return props.presences?.userPresences.map((item) => ({
  //     id: item.id,
  //     status: item.status,
